...
describe('auth routes', () => {
  ...
  it('POST /api/auth/register はオブジェクト以外のJSONで422を返す', async () => {
    ...
    try {
      const response = await testApp.app.request(createJsonRequest('/api/auth/register', 'invalid-body'))
      const body = await readJson<ValidationErrorBody>(response)
  ...
  it('POST /api/auth/register は構文不正JSONで422を返す', async () => {
    ...
    try {
      const response = await testApp.app.request(createMalformedJsonRequest('/api/auth/register', '{'))
      const body = await readJson<ValidationErrorBody>(response)
  ...
  it('POST /api/auth/login はオブジェクト以外のJSONで422を返す', async () => {
    ...
    try {
      const response = await testApp.app.request(createJsonRequest('/api/auth/login', 'invalid-body'))
      const body = await readJson<ValidationErrorBody>(response)
  ...
  it('POST /api/auth/login は構文不正JSONで422を返す', async () => {
    ...
    try {
      const response = await testApp.app.request(createMalformedJsonRequest('/api/auth/login', '{'))
      const body = await readJson<ValidationErrorBody>(response)
