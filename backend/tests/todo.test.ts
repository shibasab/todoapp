...
const createMalformedJsonRequest = (pathname: string, method: 'POST' | 'PUT', body: string): Request =>
  new Request(`http://localhost${pathname}`, {
    method,
    headers: {
      'content-type': 'application/json',
...
describe('todo routes', () => {
  ...
  it('POST /api/todo/ は構文不正JSONで422を返す', async () => {
    ...
    try {
      ...
      const response = await testApp.app.request(createMalformedJsonRequest('/api/todo/', 'POST', '{'), {
        headers: toAuthHeader(auth.token),
      })
      const body = await readJson<ValidationErrorBody>(response)
