...
const register = async (app: ReturnType<typeof createApp>, username: string, email: string): Promise<AuthBody> => {
...
describe('todo read api', () => {
  ...
  it('GET /api/todo/ は query フィルタで絞り込める', async () => {
    ...
    try {
      ...
      const response = await testApp.app.request('/api/todo/?keyword=%&progressStatus=in_progress&dueDate=today', {
        method: 'GET',
  ...
  it('GET /api/todo/ は dueDate の各フィルタで絞り込める', async () => {
    ...
    try {
      const auth = await register(testApp.app, 'todo-due-filter-user', 'todo-due-filter@example.com')
  ...
  it('GET /api/todo/ は不正progressStatusで422を返す', async () => {
    ...
    try {
      const auth = await register(testApp.app, 'query-status-user', 'query-status-user@example.com')
