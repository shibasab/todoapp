...
describe('todo usecases', () => {
  ...
  it('get/list: not found, internal, keyword filterを扱う', async () => {
    ...
    const listUsecase = createListTodosUseCase({
      todoRepo: createRepoStub({
        listByOwner: async () => [baseTodo({ name: 'match', detail: 'detail' }), baseTodo({ id: 2, name: 'other' })],
      }),
        ...
        update: async (input) => {
          ...
          return ok(
            baseTodo({
              ...
              progressStatus: (input.progressStatus as TodoItem['progressStatus'] | undefined) ?? 'in_progress',
              recurrenceType: (input.recurrenceType as TodoItem['recurrenceType'] | undefined) ?? 'daily',
            }),
          )
        },
      }),
