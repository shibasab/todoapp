...
export const createAuthenticateUseCase = (dependencies: AuthenticateDependencies): AuthenticateUseCase => {
  return async (authorizationHeaderOrToken) => {
    ...
    const user = await fromPromise(dependencies.authUserRepo.findById(userId), () => toAuthUnauthorizedError())
    if (!user.ok) {
