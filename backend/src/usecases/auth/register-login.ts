...
export const createRegisterUseCase = (dependencies: RegisterAndLoginDependencies): RegisterUseCase => {
  return async (input) => {
    ...
    const token = await fromPromise(
      dependencies.tokenPort.createAccessToken({ sub: String(createdUser.data.id) }, dependencies.authConfig),
      () => toAuthInternalError(),
    )
    if (!token.ok) {
