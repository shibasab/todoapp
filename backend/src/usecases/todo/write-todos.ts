...
import type { TodoProgressStatus, TodoRecurrenceType, TodoValidationError } from '../../domain/todo/types'
import type { ClockPort } from '../../ports/clock-port'
import type { TodoRepoCreateError, TodoRepoPort, TodoRepoUpdateError } from '../../ports/todo-repo-port'
...
const toNameUniqueViolationError = (): TodoUseCaseError => toTodoValidationError(toNameUniqueViolation())
...
const mapCreateErrorToUseCaseError = (
  ...
  switch (errorValue.type) {
    case 'DuplicatePreviousTodo':
      return options.ignoreDuplicatePreviousTodo ? { ignore: true } : { ignore: false, error: toTodoInternalError() }
    case 'DuplicateActiveName':
...
const toUtcToday = (now: Date): Date => new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()))

export const createCreateTodoUseCase = (
  dependencies: Readonly<{
    todoRepo: TodoRepoPort
  }>,
): ((input: CreateTodoInput) => TaskResult<ReturnType<typeof toTodoListItem>, TodoUseCaseError>) => {
  return async (input) => {
    ...
    const duplicated = await dependencies.todoRepo.findDuplicateActiveName(input.userId, input.name)
    if (duplicated != null) {
...
): ((input: UpdateTodoInput) => TaskResult<ReturnType<typeof toTodoListItem>, TodoUseCaseError>) => {
  return async (input) => {
    ...
    if (input.name != null) {
      const duplicated = await dependencies.todoRepo.findDuplicateActiveName(input.userId, input.name, target.id)
      if (duplicated != null) {
        return err(toNameUniqueViolationError())
      }
    }
    ...
    if (target.parentId == null && target.progressStatus !== 'completed' && nextProgressStatus === 'completed') {
      const incompleteSubtask = await dependencies.todoRepo.findIncompleteSubtask(target.id, input.userId)
...
export const createDeleteTodoUseCase = (
  ...
  return async (input) => {
    ...
    const deleted = await fromPromise(dependencies.todoRepo.deleteById(input.todoId, input.userId), () =>
      toTodoInternalError(),
    )
    if (!deleted.ok) {
