...
const normalizeKeyword = (keyword: string | undefined): string => (keyword == null ? '' : keyword.trim())
...
): ((input: ListTodosInput) => TaskResult<readonly ReturnType<typeof toTodoListItem>[], TodoUseCaseError>) => {
  return async (input) => {
    const execution = await fromPromise(
      (async () => {
        ...
        const keywordFilteredTodos =
          keyword === '' ? todos : todos.filter((todo) => todo.name.includes(keyword) || todo.detail.includes(keyword))
