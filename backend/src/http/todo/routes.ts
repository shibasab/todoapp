...
import { createTodoBodySchema, listTodoQuerySchema, todoIdParamSchema, updateTodoBodySchema } from './schemas'
...
export const createTodoHttpRoutes = (dependencies: TodoHttpRouteDependencies): Hono => {
  ...
  router.get('/', async (context) => {
    ...
    const result = await listTodos({
      ...
      ...(parsedQuery.data.progressStatus === undefined ? {} : { progressStatus: parsedQuery.data.progressStatus }),
      ...(parsedQuery.data.dueDate === undefined ? {} : { dueDateFilter: parsedQuery.data.dueDate }),
  ...
  router.put('/:todoId', async (context) => {
    ...
    const result = await updateTodo({
      ...
      ...(parsedBody.data.dueDate === undefined ? {} : { dueDate: toDateOrNull(parsedBody.data.dueDate) }),
      ...(parsedBody.data.progressStatus === undefined ? {} : { progressStatus: parsedBody.data.progressStatus }),
      ...(parsedBody.data.recurrenceType === undefined ? {} : { recurrenceType: parsedBody.data.recurrenceType }),
    })
    if (!result.ok) {
      return respondError(context, result.error)
    }
