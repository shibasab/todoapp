...
export const flatMap = <T, E, U, F>(result: Result<T, E>, mapper: (value: T) => Result<U, F>): Result<U, E | F> => {
  if (!result.ok) {
    return result
  }
...
export const fromPromise = async <T, E>(promise: Promise<T>, mapRejected: (error: unknown) => E): TaskResult<T, E> => {
  try {
